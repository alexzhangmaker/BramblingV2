...
class AccountingProcess {
  constructor() {
    ...
    // 市场到货币和交易所的映射
    this.marketMapping = {
      'CN': { currency: 'CNY', exchange: 'CN' },
      'HK': { currency: 'HKD', exchange: 'HK' },
      'UK': { currency: 'GBP', exchange: 'UK' },
      'CA': { currency: 'CAD', exchange: 'CA' },
      'US': { currency: 'USD', exchange: 'US' },
      'NASDAQ': { currency: 'USD', exchange: 'US' },
      'NYSE': { currency: 'USD', exchange: 'US' }
      // 可以继续添加其他市场映射
    };
  }
  ...
  getCurrencyAndExchange(market) {
    ...
    // 默认值
    console.log(`⚠️ 未知的市场: ${market}，使用默认值 USD/US`);
    return { currency: 'USD', exchange: 'US' };
  }
  ...
  async handleClearDeal(dealRecord) {
    ...
    console.log(`\n🔄 开始处理交易记录 #${dealRecord.dealID}: ${dealRecord.account} - ${dealRecord.action} ${dealRecord.quantity} ${dealRecord.ticker} (${dealRecord.market})`);
  ...
  async validateDealRecord(dealRecord) {
    ...
    const requiredFields = ['dealID', 'account', 'action', 'ticker', 'quantity', 'price', 'market'];
    ...
    // 验证市场是否支持
    const { currency, exchange } = this.getCurrencyAndExchange(dealRecord.market);
    console.log(`   市场映射: ${dealRecord.market} -> ${currency}/${exchange}`);
  ...
  async updateExistingHolding(accountTable, existingHolding, dealRecord) {
    ...
    // 只有在货币或交易所发生变化时才更新这些字段
    if (existingHolding.currency !== currency || existingHolding.exchange !== exchange) {
      updateData.currency = currency;
      updateData.exchange = exchange;
      console.log(`   更新货币/交易所: ${existingHolding.currency}/${existingHolding.exchange} -> ${currency}/${exchange}`);
    }
    ...
    console.log(`   ✅ 持仓更新成功: ${dealRecord.ticker} -> ${newHolding}股 @ $${newCostPerShare.toFixed(2)} ${currency}`);
    return true;
  }
  ...
  async createNewHolding(accountTable, dealRecord) {
    ...
    console.log(`   货币/交易所映射: ${dealRecord.market} -> ${currency}/${exchange}`);
    ...
    console.log(`   ✅ 新持仓创建成功: ${dealRecord.ticker} - ${dealRecord.quantity}股 @ $${dealRecord.price} ${currency}`);
    return true;
  }
  ...
  async processLedgerTable(dealRecord) {
    ...
    // 根据 market 字段获取货币
    const { currency } = this.getCurrencyAndExchange(dealRecord.market);
    
    // 现金记录的 assetID 包含货币信息
    const assetID = `${dealRecord.account}_CASH_${currency}`;
    ...
    try {
      ...
      if (queryError && queryError.code === 'PGRST116') {
        ...
        return await this.createNewCashRecord(assetID, dealRecord, newCashBalance, currency);
      } else if (queryError) {
        throw queryError;
      } else {
  ...
  // 创建新的现金记录，支持不同货币
  async createNewCashRecord(assetID, dealRecord, cashBalance, currency) {
    const { error } = await this.supabase
      .from('ledger')
      .insert({
        assetID: assetID,
        AssetType: 'cash',
        Currency: currency,  // 使用正确的货币
        Cash: cashBalance,
        Debt: 0,
        marketValueCNY: 0,
        ValueTTMCNY: 0,
    ...
    console.log(`   ✅ 创建现金记录成功: ${assetID}, 现金${actionText} ${Math.abs(cashBalance).toFixed(2)} ${currency}`);
    return true;
  }
  ...
  async getCompanyName(ticker) {
    const companyMap = {
      ...
      'APO': 'Apollo Global Management', 'VYM': 'Vanguard High Dividend Yield ETF'
    };
  ...
  async showAccountStats() {
    ...
    // 获取各账户的未清算交易数量
    const unclearedDeals = await this.getUnclearedDeals(1000);
    if (unclearedDeals) {
      const accountStats = {};
  ...
  showMarketMapping() {
    ...
    Object.entries(this.marketMapping).forEach(([market, config]) => {
      console.log(`  ${market} -> ${config.currency}/${config.exchange}`);
    });
  }
}
