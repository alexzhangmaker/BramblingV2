...
async function initializeDatabase() {
  ...
  try {
   ...
   // 更新表结构，添加新字段
    ...
    `);
    console.log('✅ 创建/更新 tblAccountHoldings 表');
    ...
    for (const [holdingKey, holding] of Object.entries(holdings)) {
      try {
        await safeDuckDBRun(connection, `
          ...
          (accountID, ticker, company, costPerShare, currency, holding, lastUpdated)
        ...
        console.log(`✅ 处理持仓: ${accountID} - ${holding.ticker}`);
      } catch (error) {
...
async function checkDatabaseStatus() {
  ...
  try {
    ...
    /*
    // 显示持仓数据
    const holdings = await safeDuckDBQuery(connection, 'SELECT accountID, ticker, holding, costPerShare, currency FROM tblAccountHoldings');
    console.log('\n📋 当前持仓:');
    holdings.forEach(h => {
      console.log(`  ${h.accountID} - ${h.ticker}: ${h.holding}股 @ ${h.costPerShare} ${h.currency}`);
    });
    */
   // 显示持仓数据 - 更新显示新字段
