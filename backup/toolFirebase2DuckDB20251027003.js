...
class DatabaseInitializer {
        ...
        if (accountData) {
          try {
            ...
            await this.safeRun(connection, `
              ...
              (accountID, baseCurrency, cashOriginal, debtOriginal, cashCNY, debtCNY, securitiesValueCNY, totalValue)
              VALUES (?, ?, ?, ?, 0, 0, 0, 0)
  ...
  async verifyDatabase() {
    ...
    try {
      ...
      // 显示资产负债表统计
      const balanceStats = await this.safeQuery(connection, `
        SELECT accountID, baseCurrency, cashOriginal, debtOriginal, cashCNY, debtCNY, securitiesValueCNY, totalValue
      ...
      balanceStats.forEach(stat => {
        console.log(`  ${stat.accountID}: 现金 ${stat.cashOriginal} ${stat.baseCurrency}, 负债 ${stat.debtOriginal} ${stat.baseCurrency}`);
      });
