 ...
 * 更新资产价格（使用指定客户端）
 */
async function updateAssetPrice(assetId, symbol, client = supabase) {
    try {
        const today = new Date().toISOString().split('T')[0];
        
        const existingPrice = await checkPriceExists(assetId, today, client);
        
        if (existingPrice) {
            console.log(`资产 ${symbol} 今天已有价格记录，跳过更新`);
            return {
                success: true,
                action: 'skipped',
                assetId: assetId,
        ...
        const quotePrice = await yAPIModule.API_FetchQuote(symbol);
        
        if (quotePrice === null || quotePrice === undefined) {
            throw new Error(`无法获取股票 ${symbol} 的报价`);
        }
        
        const assetInfo = await getAssetInfo(assetId, client);
        const priceResult = await insertAssetPrice(assetId, quotePrice, assetInfo.currency, today, client);
        ...
        console.error('更新资产价格时发生错误:', error);
        throw error;
    }
}
 ...
 * 检查价格记录是否已存在（使用指定客户端）
 */
async function checkPriceExists(assetId, priceDate, client = supabase) {
    try {
        const { data, error } = await client
            .from('asset_prices')
            .select('id')
        ...
        console.error('检查价格存在性时发生错误:', error);
        throw error;
    }
}
 ...
 * 获取资产信息（使用指定客户端）
 */
async function getAssetInfo(assetId, client = supabase) {
    try {
        const { data, error } = await client
            .from('assets')
        ...
        if (!data) {
            throw new Error(`未找到资产ID: ${assetId}`);
        }
 ...
 * 批量处理多个股票代码
 */
async function populateQuotesBatch(tickers) {
    try {
        const results = [];
        
        // 首先获取认证客户端（在整个批量过程中使用同一个认证会话）
        const authSupabase = await getAuthSupabase();
        ...
        for (const ticker of tickers) {
            try {
                ...
                if (typeof ticker === 'string') {
                    symbol = ticker;
                    exchange = null;
                } else if (typeof ticker === 'object' && ticker.symbol) {
                    symbol = ticker.symbol;
                    exchange = ticker.exchange || null;
                } else {
                    console.error('无效的股票代码格式:', ticker);
                    results.push({
                        success: false,
                        symbol: 'unknown',
                        error: '无效的股票代码格式'
                ...
                const result = await populateQuote(symbol, exchange);
                results.push(result);
                
                await new Promise(resolve => setTimeout(resolve, 100));
            } catch (error) {
                console.error(`处理股票代码 ${ticker} 时发生错误:`, error);
                const symbol = typeof ticker === 'string' ? ticker : (ticker.symbol || 'unknown');
                results.push({
                    success: false,
                    symbol: symbol,
                    error: error.message
        ...
        console.error('批量处理股票代码时发生错误:', error);
        throw error;
    }
}
